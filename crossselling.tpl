		...
		<ul {if count($orderProducts) > 4}style="width: {math equation="width * nbImages" width=58 nbImages=$orderProducts|@count}px"{/if}>
			...
			<li>
				...
				{else}
					<br />
				{/if}
				<!-- <a title="{l s='View' d='Shop.Theme.Actions'}" href="{$orderProduct.link}" class="button_small">{l s='View' d='Shop.Theme.Actions'}</a><br /> -->
			</li>
			{/foreach}
		</ul>
	</div>
	<a id="blockcart_scroll_right" class="blockcart_scroll_right{if count($orderProducts) < 5} hidden{/if}" title="{l s='Next' d='Shop.Theme'}" rel="nofollow">{l s='Next' d='Shop.Theme'}</a>
{/if}
